...
## 2026-08-10：按天统计改为跟随系统时区

决策：从 v0.7 起，按天 Token 和项目用量默认跟随运行 App 的 Mac 系统时区，不再固定 UTC+8。完整看板增加近 2～7 天、14 天、30 天、90 天、上一整周、全部和自定义日期范围。

原因：用户需要与当前电脑日期直觉一致，并查看任意连续自然日或完整自然周。时区变更只影响日期归属，不改变 Token、价格、模型归并、项目归因、去重或 cache 口径。

边界：v0.7 不做运行时多时区切换；该能力需要多套按日聚合或重新扫描日志，后续单独评估。
